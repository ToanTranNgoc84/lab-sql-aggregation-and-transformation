...
SELECT rental_id, customer_id, rental_id, DATE_FORMAT(rental_date, "%M") AS rental_month , DATE_FORMAT(rental_date, "%W") AS rental_weekday  FROM rental;
...
SELECT rental_id, customer_id, rental_date, DATE_FORMAT(rental_date, '%M') AS rental_month, DATE_FORMAT(rental_date, '%W') AS rental_weekday,
    CASE 
...
SELECT rating, COUNT(*) AS films_rating FROM film GROUP BY rating;

-- 1.3 The number of films for each rating, sorting the results in descending order of the number of films. 
-- This will help you to better understand the popularity of different film ratings and adjust purchasing decisions accordingly.
SELECT rating, COUNT(*) AS film_ratings FROM film GROUP BY rating ORDER BY film_ratings DESC;
...
SELECT rating, ROUND(AVG(length), 2) AS mean_duration FROM film GROUP BY rating HAVING AVG(length) > 120 ORDER BY mean_duration DESC;
